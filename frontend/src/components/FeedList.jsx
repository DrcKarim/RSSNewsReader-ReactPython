...
export default function FeedList() {
    ...
    const handleRefreshFeed = async (feedId) => {
       // if (!selectedFeed) return;
        ...
        try {
            ...
            const res = await getFeedItems(feedId);
            setItems(res.data);  // or res.data.items, depending on your API shape
        } catch (err) {
    ...
    const handleDeleteFeed = async () => {
        if (!selectedFeed) return;
        const confirmed = window.confirm(`Are you sure you want to delete "${selectedFeed.title}"?`);
        if (!confirmed) return;

        try {
            await deleteFeed(selectedFeed.id); // <- Add this in api.js
    ...
    return (
        <div className="layout" style={{display: 'flex', height: 'calc(100vh - 60px)'}}>
            {/* Sidebar */}
            ...
            <div style={{flex: 1, padding: '20px', overflowY: 'auto'}}>
                <div className="feed-header">
                    ...
                    <div className="feed-actions">
                        <button onClick={() => handleRefreshFeed(selectedFeed.id)}>🔁</button>
                        <button title="Edit" onClick={handleEditFeed}>✏️</button>
                        <button title="Delete" onClick={handleDeleteFeed}>🗑️</button>
                    </div>
                </div>
                {Array.isArray(items) && items.length === 0 && <p>No items found or loading...</p>}
                {Array.isArray(items) && items.map((item) => (
                    <div key={item.id} style={{marginBottom: '20px'}}>
                        ...
                        <small>{new Date(item.published_date).toLocaleString()}</small>
                        <hr/>
                    </div>
                ))}
            </div>
